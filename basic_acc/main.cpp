...
public:
	...
	addr_t sobelFilter_start_addr2 = 0x73000004;
...
int sc_main(int argc, char **argv)
{
	...
	MemoryDMI dmi = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
	InstrMemoryProxy instr_mem(dmi, core1);

	std::shared_ptr<BusLock> bus_lock = std::make_shared<BusLock>();
	core0_mem_if.bus_lock = bus_lock;
	core1_mem_if.bus_lock = bus_lock;

	instr_memory_if *instr_mem_if = &core0_mem_if;
	data_memory_if *data_mem_if = &core0_mem_if;
